...
const LoadTesterConfig = ({ data }) => {
  ...
  const [stateTenantList, dispatchTenantList] = useReducer(
    reducerTenantList,
  ...
  const handleTextChange = (e, type, i) => {
    console.log(
      `inside handleTextChange, payloadType is: ${typeof e.target
        .value} and value is ${e.target.value} }`
    );
    dispatch({
      type: type,
  ...
  return (
    <>
      <form
        className="form bg-light text-dark table-space"
        onSubmit={handleSubmit(onSubmit)}
      >
        <h2 className="text-center p-2 mb-2 bg-primary text-white margin-top">
        ...
        <TenantList
          stateTenantList={stateTenantList}
          updateTenantList={updateTenantList}
          deleteTenant={deleteTenant}
          DeleteTenantButton={DeleteTenantButton}
          register={register}
          errors={errors}
        />
